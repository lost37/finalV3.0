...
namespace
{
    ...
    constexpr int RED_MAX_AREA = 930;  // 调参：红色轮廓最大面积，超过认为不是需要绕行的红色色块，忽略并保持普通巡线。
    ...
    constexpr int REDBLOCK_VISUAL_TANGENT_NEAR_OFFSET = 24;
    ...
    constexpr int32_t REDBLOCK_VISUAL_SEEK_SPEED_CMD = 125;
    // 调参：快速靠边阶段最大差速，增大可更快抵达边线，过大可能过冲。
    constexpr float REDBLOCK_VISUAL_SEEK_DIF_LIMIT = 53.0f;
    // 调参：判断已经贴近边线的误差阈值和连续帧数。
    constexpr float REDBLOCK_VISUAL_SEEK_READY_ERR = 8.0f;
    ...
    // 调参：贴边保持阶段速度和差速限幅。
    constexpr int32_t REDBLOCK_VISUAL_HOLD_SPEED_CMD = 150;
    constexpr float REDBLOCK_VISUAL_HOLD_DIF_LIMIT = 28.0f;
    // 调参：确认贴边后沿边线行驶的编码器距离，不包含前面的靠边距离。
    constexpr int32_t REDBLOCK_VISUAL_HOLD_DISTANCE = 5000;
    ...
    for(row = 0; row < Cut_ROW; row++)
    {
        ...
        if(redblock_bypass_phase == RB_BYPASS_PHASE_SEEK_BOUNDARY)
        {
            ...
            const float boundary_blend =
                1.0f - powf(1.0f - tangent_progress, tangent_power);
