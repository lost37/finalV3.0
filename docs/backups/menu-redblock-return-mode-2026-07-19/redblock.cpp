...
volatile int redblock_detection_enable = 1;
volatile int32_t redblock_bypass_speed_cmd = 0;
volatile int32_t redblock_slowdown_speed_cmd = 90;  //红块减速速度

namespace
{
    enum RedBlockVisualReturnMode : uint8
    {
        RB_VISUAL_RETURN_SMOOTH_BLEND = 0,
        RB_VISUAL_RETURN_LEGACY_DIRECT,
    };

    // 调参：回线方式。0=当前平滑混合回中线；1=旧版直接恢复普通中线。
    // constexpr RedBlockVisualReturnMode REDBLOCK_VISUAL_RETURN_MODE = RB_VISUAL_RETURN_SMOOTH_BLEND;//RB_VISUAL_RETURN_LEGACY_DIRECT
    constexpr RedBlockVisualReturnMode REDBLOCK_VISUAL_RETURN_MODE = RB_VISUAL_RETURN_LEGACY_DIRECT;//RB_VISUAL_RETURN_LEGACY_DIRECT
    ...
    constexpr int RED_MIN_AREA = 103;
    ...
    constexpr float REDBLOCK_SIDE_IGNORE_RATIO = 0.17f;  // 调参：仅赛道最外侧 12% 的红色轮廓才会被视为贴边红砖。
    ...
    constexpr int32_t REDBLOCK_VISUAL_POST_PASS_HOLD_DISTANCE = 4600;
