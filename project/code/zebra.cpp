...
// 菜单开关：0=斑马线只识别和计圈，不停车；1=到目标圈数后停车。
volatile int zebra_stop_enable = 1;
...
// 调参：检测到目标斑马线后继续运行的帧数。43 帧后停车，不依赖编码器值。
static const uint8 ZEBRA_STOP_DELAY_FRAMES = 60;
static uint8 zebra_stop_delay_active = 0;
static uint8 zebra_stop_delay_frames = 0;
...
static void Zebra_StartDelayedStop(void)
{
    ...
    if(zebra_stop_delay_active == 0)
    {
        ...
        zebra_stop_delay_frames = 0;
        printf("[ZEBRA] detected -> delay_stop frames=%u\n",
               (unsigned)ZEBRA_STOP_DELAY_FRAMES);
...
static void Zebra_UpdateDelayedStop(void)
{
    if(zebra_stop_delay_active != 0)
    {
        if(zebra_stop_delay_frames < ZEBRA_STOP_DELAY_FRAMES)
        {
            zebra_stop_delay_frames++;
        }
        if(zebra_stop_delay_frames >= ZEBRA_STOP_DELAY_FRAMES)
        {
            ...
            zebra_flag = 3;
            stop = 1;
            printf("[ZEBRA] delay_stop_done frames=%u\n",
                   (unsigned)zebra_stop_delay_frames);
        }
    }
}
