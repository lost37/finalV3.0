...
uint8_t TuningMenu_Register(MenuCore *menu, TuningMenuBindings *bindings)
{
    ...
    MenuCoreNode *stop_control = MenuCore_AddFolder(menu, root, "Stop");

    if(pid == nullptr || camera == nullptr || speed == nullptr || ack == nullptr || redblock == nullptr || stop_control == nullptr)
    {
        return 0;
    }
    ...
    MenuCore_AddInt(menu, redblock, "cross_fill", MutableInt(bindings->redblock_cross_fill_enable), 1.0f, 0.0f, 1.0f);
    MenuCore_AddInt(menu, stop_control, "boundary", MutableInt(bindings->boundary_stop_enable), 1.0f, 0.0f, 1.0f);
    MenuCore_AddInt(menu, stop_control, "zebra", MutableInt(bindings->zebra_stop_enable), 1.0f, 0.0f, 1.0f);
